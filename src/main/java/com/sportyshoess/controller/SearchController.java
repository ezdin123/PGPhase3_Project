package com.sportyshoess.controller;
...
import com.sportyshoess.service.SportyShoesService;
...
public class SearchController {
	...
	public ResponseEntity<List<PurchaseReport>> getAllPurchaseReportsByCategory(@PathVariable String category) {
		List<PurchaseReport> theCustomer = service.getAllPurchaseReportsByCategory(category);
		if (theCustomer == null) {
			throw new BusinessException("category - " + category);
		}
		return new ResponseEntity<List<PurchaseReport>>(service.getAllPurchaseReportsByCategory(category),
				HttpStatus.FOUND);
	}
